...
class RevSliderOutput
{
    ...
    private function putSlides($gal_ids = array(), $order = array())
    {
        ...
        if ($do_static && !empty($this->static_slide)) {
            ...
            if (!empty($layers)) {
                ...
                if (!empty($staticlayserpos) && $staticlayserpos === 'back') {
                    $staticlayserposition = ' tp-static-layers-back';
                }
    ...
    public function putBackgroundVideo($slide)
    {
        ...
        if ($this->adv_resp_sizes == true) {
            ...
            if (is_object($videoWidth)) {
                $videoWidth = RevSliderFunctions::get_biggest_device_setting($videoWidth, $this->enabled_sizes);
            }
            if (is_object($videoHeight)) {
                $videoHeight = RevSliderFunctions::get_biggest_device_setting($videoHeight, $this->enabled_sizes);
            }
        }
    ...
    public function putLayer($layer, $row_group_uid = false, $static_slide = false, $special_type = false)
    {
        ...
        if (!isset($layer['deformation'])) {
            $layer['deformation'] = array();
        }
        if (!isset($layer['deformation-hover'])) {
            $layer['deformation-hover'] = array();
        }
        ...
        if (trim($class) !== '') {
            ...
            if (!isset($this->in_class_usage[trim($class)])) {
                $this->in_class_usage[trim($class)] = RevSliderOperations::getCaptionsContentArray(trim($class));
            }
        }
        ...
        **/
        ...
        switch ($type) {
            ...
            case 'button':
                ...
                if (!empty($static_styles)) {
                    ...
                    if (!empty($static_styles['letter-spacing'])) {
                        ...
                        if ($mcls !== $dls) {
                            $inline_styles .= ' letter-spacing: '.$mcls.';';
                        }
                    }
                }
            ...
            case 'image':
                ...
                if ($this->adv_resp_sizes == true) {
                    ...
                    if ($scaleX !== '') {
                        ...
                        if ($y_is_single) { //force to array if voffset is also array
                            if (!isset($myScaleX)) {
                                $myScaleX = RevSliderFunctions::get_biggest_device_setting($scaleX, $this->enabled_sizes);
                            }
                    ...
                    if ($scaleY !== '') {
                        ...
                        if ($y_is_single) { //force to array if voffset is also array
                            if (!isset($myScaleY)) {
                                $myScaleY = RevSliderFunctions::get_biggest_device_setting($scaleY, $this->enabled_sizes);
                            }
                ...
                if (isset($this->slide->ignore_alt)) {
                    $alt = '';
                }
        ...
        if ($type==="row") {
            ...
            if ($columnbreak==="desktop") {
                $columnbreak = 0;
            }
            if ($columnbreak==="notebook") {
                $columnbreak = 1;
            }
            if ($columnbreak==="tablet") {
                $columnbreak = 2;
            }
            if ($columnbreak==="mobile") {
                $columnbreak = 3;
            }
        }
        ...
        if ($this->adv_resp_sizes == true) {
            ...
            if (!empty($static_styles)) {
                ...
                if (!empty($static_styles['letter-spacing'])) {
                    if (is_object($static_styles['letter-spacing'])) {
                        ...
                        if (strpos($ss_ls, '[') !== false) {
                            $static_data .= str_replace('px', '', '			data-letterspacing="'.$ss_ls.'"')."\n";
                        }
                    }
                }
            }
            ...
            if (is_object($alignHor)) {
                $alignHor = RevSliderFunctions::get_biggest_device_setting($alignHor, $this->enabled_sizes);
            }
            if (is_object($alignVert)) {
                $alignVert = RevSliderFunctions::get_biggest_device_setting($alignVert, $this->enabled_sizes);
            }
            if (is_object($left)) {
                $left = RevSliderFunctions::get_biggest_device_setting($left, $this->enabled_sizes);
            }
            if (is_object($top)) {
                $top = RevSliderFunctions::get_biggest_device_setting($top, $this->enabled_sizes);
            }
        ...
        if (strpos($calc_bg_color, "gradient")!==false || strpos($calc_dbc_color, "gradient")!==false) {
            $st_idle['background'] = array(TPColorpicker::convert($bg_color, $bg_trans), TPColorpicker::convert($dbc, $dbt));
        } //'ALWAYS'
        
        else {
            $st_idle['background-color'] = array(TPColorpicker::convert($bg_color, $bg_trans), TPColorpicker::convert($dbc, $dbt));
        } //'ALWAYS'
        ...
        if ($bdw == '0px' || $bdw == '0px 0px 0px 0px') {
        ...
        if (!empty($my_border) && is_array($my_border)) {
            ...
            if ($my_border == '0px' || $my_border == '0px 0px 0px 0px') {
                unset($st_idle['border-style']);
            }
            if (trim($my_border) != '') {
        ...
        if (
            ...
            if (isset($st_idle['border-width'])) {
                unset($st_idle['border-width']);
            }
            if (isset($st_idle['border-color'])) {
                unset($st_idle['border-color']);
            }
            if (isset($st_idle['border-style'])) {
                unset($st_idle['border-style']);
            }
        ...
        foreach ($st_idle as $key => $value) {
            if ($type == 'image' || $type == 'video') { //do not print unneeded styles
                if (in_array($key, $this->ignore_styles)) {
                    continue;
                }
            }
            if (trim($value[0]) == '' || $value[0] == $value[1]) {
        ...
        if ($columnbreak != '') {
            echo '			data-columnbreak="'.$columnbreak.'"'." \n";
        }
    ...
    public function set_placeholders($text)
    {
        global $post;
        if (isset($post->ID)) {
            $text = str_replace(array('%current_page_link%', '{{current_page_link}}'), get_permalink($post->ID), $text);
        }
        $text = str_replace(array('%home_url%', '{{home_url}}'), esc_url(home_url('/')), $text);
        if (isset($post->ID)) {
            $text = apply_filters('revslider_gallery_set_placeholders', $text, $post->ID);
        }
        return $text;
    }
    ...
    public function putCreativeZones()
    {
        ...
        foreach ($zones as $zs => $zone) { //go through all three zones
            $lzone = array();
            foreach ($layers as $layer) {
                if (RevSliderFunctions::getVal($layer, 'type', 'text') !== 'row') {
                    continue;
                } //we only want to handle rows here to get the zones we need to create
    ...
    public function putCreativeRows($layers, $current_zone)
    {
        ...
        foreach ($layers as $layer) {
            if (RevSliderFunctions::getVal($layer, 'type', 'text') !== 'row') {
                continue;
            } //we only want to handle rows here of the current zone and add them as a rows
            
            $layer_zone = RevSliderFunctions::getVal($layer, 'align_vert');
            $layer_zone = (is_object($layer_zone)) ? RevSliderFunctions::get_biggest_device_setting($layer_zone, $enabled_sizes) : $layer_zone;
            
            if ($layer_zone !== $current_zone) {
                continue;
            } //wrong zones, so continue
    ...
    public function putCreativeColumn($layers, $unique_id)
    {
        ...
        foreach ($layers as $layer) {
            ...
            if (RevSliderFunctions::getVal($layer, 'type', 'text') !== 'column') {
                continue;
            } //we only want to handle columns here of the current row
            if (RevSliderFunctions::getVal($layer, 'p_uid', -1) !== $unique_id) {
                continue;
            } //has the wrong row ID
    ...
    public function putCreativeGroups()
    {
        ...
        foreach ($layers as $layer) {
            if (RevSliderFunctions::getVal($layer, 'type', 'text') !== 'group') {
                continue;
            } //we only want to handle groups here to get the zones we need to create
