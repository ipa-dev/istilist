    ...
    public function write_template_markup($template, $slider_id = false)
    {
		...
		<div class="template_slide_single_element" style="display:inline-block">
            ...
            ?>
				...
				<span data-sliderid="<?php echo $slider_id; ?>" data-slideid="<?php echo $slide_id; ?>" class="show_more_template_slider <?php if (isset($template["user_template"])) {
        ...
        if (isset($template["user_template"])) {
            ...
            //allow / disallow download ?>
                ...
                if ($allow_install !== false) {
					...
					<i class="eg-icon-plus"></i><?php if ($slider_id == false) {
        ...
        if (!empty($tp_template_slider)) {
            foreach ($tp_template_slider as $m_slider) {
                ...
                if (!isset($m_slider['installed']) && !isset($m_slider['is_new'])) {
                    ...
                    if (!empty($c_slides)) {
                        ?>
						<div style="margin-bottom:30px" class="template_group_wrappers  <?php echo $slidercat.$slidercat_new.' ';
                    ...
                    if (!empty($c_slides)) {
                        ?>
						<div style="margin-bottom:30px"  class="template_group_wrappers not-imported-wrapper <?php echo $slidercat.$slidercat_new;
                        if (isset($m_slider['filter'])) {
                            echo implode(' ', $m_slider['filter']);
                        } ?>">
        ...
        if (!empty($tp_template_slider)) {
            foreach ($tp_template_slider as $isd => $m_slider) {
                ...
                if (!isset($m_slider['installed']) && !$is_package_parent) {
                    ...
                    if (!empty($c_slides)) {
                        ?>
						<div <?php echo $datapackagegroup; ?> style="<?php echo $prestyle; ?>" class="template_group_wrappers <?php echo $slidercat.$package.$slidercat_new;
                        if (isset($m_slider['filter'])) {
                            echo implode(' ', $m_slider['filter']);
                        } ?>">
							<?php
                    ...
                    ?>
					<div <?php echo $datapackagegroup; ?> style="<?php echo $prestyle; ?>" class="template_group_wrappers <?php echo $slidercat_new.$slidercat.$package; ?> temp_notinstalled not-imported-wrapper <?php if (isset($m_slider['filter'])) {
                        echo implode(' ', $m_slider['filter']);
                    } ?>">
						<?php
