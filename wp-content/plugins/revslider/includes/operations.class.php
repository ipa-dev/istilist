...
class RevSliderOperations extends RevSliderElementsBase
{
    ...
    public static function parseCustomAnimationByArray($animArray, $is = 'start', $frame_val)
    {
        ...
        if (isset($animArray['x_'.$is]) && $animArray['x_'.$is] !== '' && $animArray['x_'.$is] !== 'inherit') {
            $retString.= 'x:'.$animArray['x_'.$is].$reverse.';';
        } //movex
        $reverse = (isset($animArray['y_'.$is.'_reverse']) && $animArray['y_'.$is.'_reverse'] == true) ? '(R)' : ''; //movey reverse
        if (isset($animArray['y_'.$is]) && $animArray['y_'.$is] !== '' && $animArray['y_'.$is] !== 'inherit') {
            $retString.= 'y:'.$animArray['y_'.$is].$reverse.';';
        } //movey
        if (isset($animArray['z_'.$is]) && $animArray['z_'.$is] !== '' && $animArray['z_'.$is] !== 'inherit') {
        ...
        if (isset($animArray['x_rotate_'.$is]) && $animArray['x_rotate_'.$is] !== '' && $animArray['x_rotate_'.$is] !== 'inherit') {
        ...
        if (isset($animArray['y_rotate_'.$is]) && $animArray['y_rotate_'.$is] !== '' && $animArray['y_rotate_'.$is] !== 'inherit') {
        ...
        if (isset($animArray['z_rotate_'.$is]) && $animArray['z_rotate_'.$is] !== '' && $animArray['z_rotate_'.$is] !== 'inherit') {
        ...
        if (isset($animArray['skew_x_'.$is]) && $animArray['skew_x_'.$is] !== '' && $animArray['skew_x_'.$is] !== 'inherit') {
        ...
        if (isset($animArray['skew_y_'.$is]) && $animArray['skew_y_'.$is] !== '' && $animArray['skew_y_'.$is] !== 'inherit') {
    ...
    public static function parseCustomMaskByArray($animArray, $is = 'start')
    {
        ...
        if (isset($animArray['mask_x_'.$is]) && $animArray['mask_x_'.$is] !== '') {
            $retString.= 'x:'.$animArray['mask_x_'.$is].$reverse.';';
        }
        $reverse = (isset($animArray['mask_y_'.$is.'_reverse']) && $animArray['mask_y_'.$is.'_reverse'] == true) ? '(R)' : '';
        if (isset($animArray['mask_y_'.$is]) && $animArray['mask_y_'.$is] !== '') {
            $retString.= 'y:'.$animArray['mask_y_'.$is].$reverse.';';
        }
        if (isset($animArray['mask_speed_'.$is]) && $animArray['mask_speed_'.$is] !== '') {
            $retString.= 's:'.$animArray['mask_speed_'.$is].';';
        }
        if (isset($animArray['mask_ease_'.$is]) && $animArray['mask_ease_'.$is] !== '') {
     ...
     */
    public function getArrCaptionSorted($contentCSS)
    ...
    public function insertCaptionsContentData($content)
    {
        ...
        if (!isset($content['handle']) || !isset($content['idle']) || !isset($content['hover'])) {
            return false;
        } // || !isset($content['advanced'])
        
        $db = new RevSliderDB();

        $handle = $content['handle'];
        
        if (!isset($content['hover'])) {
            $content['hover'] = '';
        }
        if (!isset($content['advanced'])) {
            $content['advanced'] = array();
        }
        if (!isset($content['advanced']['idle'])) {
            $content['advanced']['idle'] = array();
        }
        if (!isset($content['advanced']['hover'])) {
    ...
    public static function getCaptionsCssContentArray()
    {
        ...
        } elseif (file_exists(RS_PLUGIN_PATH.'public/assets/css/captions-original.css')) {
            $contentCSS = file_get_contents(RS_PLUGIN_PATH.'public/assets/css/captions-original.css');
        } elseif (file_exists(RS_PLUGIN_PATH.'backup/'.'captions.css')) {
            $contentCSS = file_get_contents(RS_PLUGIN_PATH.'backup/'.'captions.css');
        } elseif (file_exists(RS_PLUGIN_PATH.'backup/'.'captions-original.css')) {
...
$footer = ob_get_contents();
    ...
    public static function setCleanFontImport($font, $class = '', $url = '', $variants = array(), $subsets = array())
    {
        ...
        if (!empty($variants) || !empty($subsets)) {
            ...
            if (!isset($revslider_fonts[$font]['variants'])) {
                $revslider_fonts[$font]['variants'] = array();
            }
            if (!isset($revslider_fonts[$font]['subsets'])) {
                $revslider_fonts[$font]['subsets'] = array();
            }
  ...
  src: local('".$f_family."'), local('".$f_family."'), url(".$base_url.'/revslider/gfonts/'. $font_name . '/' . $font_name . '.woff2'.") format('woff2');
    ...
    public static function add_preset_setting($data)
    {
        if (!isset($data['settings']) || !isset($data['values'])) {
            return __('Missing values to add preset', 'revslider');
        }
