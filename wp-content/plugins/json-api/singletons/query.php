...
class JSON_API_Query
{
  ...
  // Default values
    ...
    public function wp_query_var($key)
    {
      ...
      'order_by' =>       'orderby'
        ...
        if ($key == 'date') {
            ...
            return $date;
        } elseif (isset($wp_translation[$key])) {
    ...
    public function get_controller()
    {
        ...
        } elseif (preg_match('/^([a-zA-Z0-9_]+)(\/|\.)[a-zA-Z0-9_]+$/', $json, $matches)) {
            return $matches[1];
        } else {
            return 'core';
        }
    ...
    public function get_method($controller)
    {
        ...
        if (empty($method)) {
            ...
            return false;
        } elseif (method_exists("JSON_API_{$controller}_Controller", $method)) {
            // Case 2: an explicit method was specified
            return $method;
        } elseif ($controller == 'core') {
