...
class JSON_API_Response
{
    ...
    public function respond($result, $status = 'ok', $http_status = 200)
    {
        ...
        } elseif (!empty($_REQUEST[$status_redirect])) {
            wp_redirect($_REQUEST[$status_redirect]);
        } elseif ($json_api->query->redirect) {
            $url = $this->add_status_query_var($json_api->query->redirect, $status);
            wp_redirect($url);
        } elseif ($json_api->query->callback) {
