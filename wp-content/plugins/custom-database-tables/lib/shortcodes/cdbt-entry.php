   ...
   **/
    public function entry_data_form()
    {
      ...
      'submit_button_label' => '', 		// @attribute string [optional] Specifies the label name of button for submitting in the entry form.
        ...
        } elseif (isset($table_option['roles']) && isset($table_option['roles']['input_role'])) {
            // As legacy v.1.x
        ...
        foreach ($table_schema as $column => $scheme) {
            ...
            if (array_key_exists('char', $detect_column_type)) {
                if (array_key_exists('text', $detect_column_type)) {
                    ...
                    if ('longtext' === $detect_column_type['text']) {
                        $rows = 20;
                    } elseif ('midiumtext' === $detect_column_type['text']) {
                        $rows = 15;
                    } elseif ('tinytext' === $detect_column_type['text']) {
                        $rows = 5;
                    } else {
                        $rows = 10;
                    }
                } elseif (array_key_exists('blob', $detect_column_type)) {
                    $input_type = 'file';
            ...
            } elseif (array_key_exists('numeric', $detect_column_type)) {
            ...
            } elseif (array_key_exists('list', $detect_column_type)) {
                ...
                unset($list_value);
            } elseif (array_key_exists('datetime', $detect_column_type)) {
                if (in_array($detect_column_type['datetime'], [ 'timestamp', 'year' ])) {
                    $input_type = 'number';
                    $element_size = ceil($scheme['max_length'] / 10) + 1;
                    $pattern = '^[0-9]+$';
                } elseif ('time' === $detect_column_type['datetime']) {
                    $input_type = 'text';
                    $element_size = 3;
