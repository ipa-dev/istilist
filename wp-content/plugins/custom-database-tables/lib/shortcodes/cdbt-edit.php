   ...
   **/
    public function editable_data_list()
    {
      ...
      'truncate_cols' => '', 			// @attribute string [optional] Specifies the comma-delimited column names if you want to specify the columns truncating the strings. e.g. "column1,column2,column3,..."
        ...
        } elseif (isset($table_option['roles']) && isset($table_option['roles']['edit_role'])) {
            // As legacy v.1.x
        ...
        if ($has_pk) {
            ...
            foreach ($table_schema as $column => $scheme) {
                if ('UNI' === strtoupper($scheme['column_key'])) {
                    ...
                    break;
                } elseif ($scheme['not_null']) {
                    if ('MUL' === strtoupper($scheme['column_key']) || 'datetime' === $scheme['type'] || 'timestamp' === $scheme['type']) {
                        $condition_keys[] = $column;
                    } // The columns often high accuracy uniqueness
                }
            }
