...
class CdbtValidator extends CommonValidator
{
   ...
   */
    ...
    public function esc_column_value($raw_value=null, $detect_type=[])
    {
        ...
        } elseif (array_key_exists('float', $detect_type)) {
            $retvar = floatval($raw_value);
        } elseif (array_key_exists('binary', $detect_type)) {
            $retvar = bindec($raw_value);
        } elseif (array_key_exists('text', $detect_type)) {
            $retvar = esc_textarea($raw_value);
        } elseif (array_key_exists('blob', $detect_type)) {
            $retvar = strval($raw_value);
        } elseif (array_key_exists('char', $detect_type)) {
            $retvar = esc_html($raw_value);
        } elseif (array_key_exists('list', $detect_type)) {
            $retvar = esc_attr($raw_value);
        } elseif (array_key_exists('datetime', $detect_type)) {
