...
{
    public $option_template = array();
  
    public $option_shortcodes = array();
  
    public $allow_file_types; // For import and export
  
    public $db_charsets;
  
    public $db_collations;
  
    public $timezone_identifiers;
    ...
    public $contribute_extends = array(); // Added since version 2.0.7
  
    public $support_email; // Added since version 2.0.9
  
    /**
     * Notification message text that can be overridden at default
     * @since 2.0.9
     */
    public $override_messages = array();
     ...
     * @return boolean True if the update success, otherwise false
     */
    public function add_new_table($table_name=null, $table_type='regular', $table_options=null)
    {
        static $message = '';
            ...
            if (! empty($search_key)) {
                foreach ($table_options['permission'] as $_key => $_values) {
                    if ($search_key === strstr($_key, '_global', true)) {
                        $table_permission += $_values;
                    } //array_merge($table_permission);
                }
            } else {
                $table_permission = $table_options['permission'];
            }
        } elseif (is_array($table_options) && array_key_exists('roles', $table_options) && ! empty($table_options['roles'])) {
