...
<?php foreach ([ 'actionUrl', 'formAction', 'displaySubmit', 'whereClause' ] as $_df_key) {
    $this->dynamic_field($fields_define[$_df_key]);
} ?>
      </div>
...
<?php if ($current_tab == 'shortcode_edit') :
  ...
  } elseif (array_key_exists('target_scid', $session_vars)) {
  ...
  if (! isset($_wall_message) && isset($this_tab_vars)) {
      ...
      foreach ($fields_define as $__key => $__val) {
          ...
          } elseif ('indexRow' === $__key || 'narrowOperator' === $__key || 'footerInterface' === $__key) {
          ...
          } elseif (in_array($__val['elementType'], [ 'text', 'textarea', 'spinbox' ])) {
