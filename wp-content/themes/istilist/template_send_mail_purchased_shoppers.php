<?php /* Template Name: Send Mail Purchased Shoppers */ ?>
<?php
...
if(isset($_POST['purchased_shopper_test_email'])){
...
if(isset($_POST['purchased_shopper_send']))	{
	$store_name = get_the_author_meta( 'display_name', $store_id );
	$from       = get_user_meta( $store_id, 'email_to_shopper', true );
...
if(isset($_POST['purchased_shopper_save']))	{
	$sql7 = "SELECT * FROM $table_name WHERE message_type = 'purchased-shoppers' and store_id = $store_id";
	$result7 = $wpdb->get_row($sql7);

	if(!empty($result7)){
		$update_query = "UPDATE $table_name SET subject = '".$_POST['purchased_shopper_subject']."', body = '".nl2br($_POST['purchased_shopper_email_body'])."' WHERE message_type = 'purchased-shoppers' and store_id = $store_id";
		$update = $wpdb->query($update_query);
	} else {
		$insert_query = "INSERT INTO $table_name SET subject = '".$_POST['purchased_shopper_subject']."', body = '".nl2br($_POST['purchased_shopper_email_body'])."', message_type = 'purchased-shoppers', store_id = $store_id";
